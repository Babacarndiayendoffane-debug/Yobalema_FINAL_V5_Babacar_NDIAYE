...
import '../booking/passenger_booking_screen_v2.dart';

class PassengerAuthScreenV2 extends StatefulWidget {
  const PassengerAuthScreenV2({super.key, required this.api});

  final ApiClient api;

  @override
  State<PassengerAuthScreenV2> createState() => _PassengerAuthScreenV2State();
}

class _PassengerAuthScreenV2State extends State<PassengerAuthScreenV2> {
    ...
    try {
      ...
      try {
        ...
        response = await _auth.register(
          phone: phone,
          password: password,
          name: _name.text.trim().isEmpty ? 'Passager Yobalema' : _name.text.trim(),
          role: 'PASSENGER',
        );
      }
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (_) => PassengerBookingScreenV2(api: widget.api, user: user),
        ),
      );
    } catch (error) {
              ...
              child: ConstrainedBox(
                ...
                child: Column(
                  children: [
                    ...
                    const Text('Mobilité moto dans la région de Kaolack.', textAlign: TextAlign.center),
